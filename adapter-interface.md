...
- **Adapter for the audit log itself**: resolved in `audit-spec.md`. Hot store is runtime-local SQLite; cold store uses the configured storage adapter via gzipped JSONL files in `audit/YYYY/MM/DD.jsonl.gz`. No dedicated audit adapter needed.
