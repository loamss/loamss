...
You'll build `source:hackernews` end-to-end: 90 lines of Go, no auth (HN's API is public), incremental sync via the "last seen item id" cursor pattern, ~5 minutes of typing once you've read this page. By the end you'll know enough to write a real provider connector — Gmail, Calendar, Drive — by mostly copying patterns.
...
We treat new in-tree connectors with the same review bar as any other runtime change:
...
Third-party connectors that *don't* land in-tree become capsules under the [`ingestor` role](../capsule-spec.md#ingestor-role) once the capsule SDK side is ready. Plan B from day one: same SPI, different deployment shape.
