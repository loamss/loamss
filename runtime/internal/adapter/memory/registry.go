...
//
//	func init() {
//	    memory.Register("memory:sqlite-vec", func() memory.Adapter {
//	        return &sqliteVec{}
//	    })
//	}
//
...
var ErrUnknownAdapter = errors.New("memory: unknown adapter")
...
// Register makes a memory adapter implementation available by id.
...
// hide real bugs.
//
// The expected id format is "memory:<name>" (e.g.,
// "memory:sqlite-vec"). Register does not enforce this; the config
// validator does.
func Register(id string, f Factory) {
	if f == nil {
		panic("memory: Register called with nil Factory for " + id)
	}
	registry.mu.Lock()
	defer registry.mu.Unlock()
	if _, exists := registry.factories[id]; exists {
		panic("memory: adapter " + id + " registered twice")
	}
