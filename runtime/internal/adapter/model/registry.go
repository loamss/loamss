...
//
//	func init() {
//	    model.Register("model:anthropic", func() model.Adapter {
//	        return &anthropic{}
//	    })
//	}
//
...
var ErrUnknownAdapter = errors.New("model: unknown adapter")
...
// Register makes a model adapter implementation available by id.
// Adapter packages call this from init(). Re-registering an id
// panics — adapter ids are global identifiers and silent overwriting
// would hide real bugs.
//
// The expected id format is "model:<name>" (e.g., "model:anthropic",
// "model:none", "model:dummy"). Register does not enforce this; the
// config validator does.
func Register(id string, f Factory) {
	if f == nil {
		panic("model: Register called with nil Factory for " + id)
	}
	registry.mu.Lock()
	defer registry.mu.Unlock()
	if _, exists := registry.factories[id]; exists {
		panic("model: adapter " + id + " registered twice")
	}
